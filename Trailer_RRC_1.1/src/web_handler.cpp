...
static void handleRoot() {
  ...
  if (activeTab != "home" && activeTab != "users" && activeTab != "outconfig") activeTab = defaultTab;
  ...
  if ((isAdmin || isOperator) && isOutConfig) {
    ...
    const char* names[3] = { "estop", "up", "down" };
    for (int r = 0; r < 3; r++) {
  ...
  if (isHome) {
    ...
    html += "if(!s.up&&!s.down)mt+=\"<span class='motion-tag tag-idle'>No Active Motion</span>\";";
  ...
  if (isOutConfig) {
    ...
    html += "if((estop!==noneVal&&estop===up)||(estop!==noneVal&&estop===down)||(up!==noneVal&&up===down)){setMsg(false,'Save failed: two motion functions cannot use the same output pin.');return;}";
...
static void handleOutConfigPost() {
  ...
  if (dupEstopUp || dupEstopDown || dupUpDown) {
  ...
  Serial.printf("Out config updated: ESTOP=%s UP=%s DOWN=%s\n",
...
static void handleMotionStatus() {
  ...
  // estop in web JSON reflects E-STOP output level (HIGH=1, LOW=0).
  int estopOutputOn = motionEstop ? 0 : 1;
  String json = "{\"estop\":" + String(estopOutputOn) +
                ",\"up\":"    + String(motionUp    ? 1 : 0) +
                ",\"down\":"  + String(motionDown  ? 1 : 0) +
